...
def upgrade() -> None:
    ...
    op.alter_column(
        "flights", "base_price", existing_type=sa.Numeric(precision=12, scale=2), nullable=True
    )
    ...
    conn.execute(
        sa.text("UPDATE flights SET destination_iata='UNK' WHERE destination_iata IS NULL")
    )
    ...
    op.alter_column(
        "flights", "amadeus_flight_id", existing_type=sa.String(length=128), nullable=False
    )
    op.alter_column("flights", "origin_iata", existing_type=sa.String(length=3), nullable=False)
    op.alter_column(
        "flights", "destination_iata", existing_type=sa.String(length=3), nullable=False
    )
    op.alter_column("flights", "carrier_code", existing_type=sa.String(length=3), nullable=False)
    op.create_index(
        op.f("ix_flights_amadeus_flight_id"), "flights", ["amadeus_flight_id"], unique=True
    )
    op.create_index(op.f("ix_flights_origin_iata"), "flights", ["origin_iata"], unique=False)
    op.create_index(
        op.f("ix_flights_destination_iata"), "flights", ["destination_iata"], unique=False
    )
...
def downgrade() -> None:
    ...
    op.alter_column(
        "flights", "base_price", existing_type=sa.Numeric(precision=12, scale=2), nullable=False
    )
